...
#define DATA_NUM 250000
#define NUM_BOUND 2147483647
//#define NUM_BOUND 10000
...
int main(void) {
...
#ifdef WRITEFILE
	FILE *SortRaw1 = fopen("SortRaw1.csv", "w");
	FILE *SortRaw2 = fopen("SortRaw2.csv", "w");
	FILE *SortRaw3 = fopen("SortRaw3.csv", "w");
	FILE *SortRaw4 = fopen("SortRaw4.csv", "w");
#endif

	/* ========== Seperate original data to (THREADS_NUM) chunks with same size and apply quick sort to each chunk ========== */
	char tmp[128] = {};
	clock_gettime(CLOCK_REALTIME, &t_start);
	#pragma omp parallel for private(i, j, tmp)
	for (i = 0; i < THREADS_NUM; i++) {
		qsort(&pRawNums[i][0], DATA_NUM, sizeof(int), compare);
		...
		switch(omp_get_thread_num())
		{
			case 0:
				for (j = 0; j < DATA_NUM; j++)
					fprintf(SortRaw1, "%d,", pRawNums[i][j]);
				break;
			case 1:
				for (j = 0; j < DATA_NUM; j++)
					fprintf(SortRaw2, "%d,", pRawNums[i][j]);
				break;
			case 2:
				for (j = 0; j < DATA_NUM; j++)
					fprintf(SortRaw3, "%d,", pRawNums[i][j]);
				break;
			case 3:
				for (j = 0; j < DATA_NUM; j++)
					fprintf(SortRaw4, "%d,", pRawNums[i][j]);
				break;
		}
#endif
...
#ifdef DEBUG	
	printf("After first chuck sort\n");
	for (i = 0; i < THREADS_NUM; i++) {
		printf("%d ******************************************************\n", i);
		for (j = 0; j < DATA_NUM; j++) {
			printf("%d, ", pRawNums[i][j]);
		}
		printf("\n");
	}
#endif
...
#ifdef WRITEFILE
	FILE *AfterSort = fopen("AfterSort.csv", "w");
	for (i = 0; i < THREADS_NUM; i++) {
		for (j = 0; j < DATA_NUM; j++) {
			fprintf(AfterSort, "%d,", pRawNums[i][j]);
		}
		fprintf(AfterSort, "\n");
	}
	fclose(AfterSort);
	...
	printf("%-40s %lf ms\n", "Seperate data into chunks:", elapsedTime);
	totalTime += elapsedTime;

	/* ========== Choose (THREADS_NUM - 1) candidates from each chunk (candidates in the same chunk should seperate chunk equally) ========== */
	clock_gettime( CLOCK_REALTIME, &t_start);
	int count = 0;
	for (i = 0; i < THREADS_NUM; i++) {
		for (j = 0; j < THREADS_NUM-1; j++) {
			//candidates[count] = pRawNums[i][(j+1) * (DATA_NUM / (THREADS_NUM)) - 1];
			float e_index = (float)DATA_NUM / (THREADS_NUM) * (j + 1);
			//printf("%d, ", (int)e_index);
			candidates[count] = pRawNums[i][(int)e_index - 1];
			count++;
		}
	}
	//printf("\n");
